...
def test_tic():
    ...
    catalog_new = query_catalog(
        coord=c, epoch=Time(2461041.500, scale="tt", format="jd"), catalog="tic", radius=80, magnitude_limit=18
    )
    ...
    catalog_new = query_TIC(
        coord=c, epoch=Time(2461041.500, scale="tt", format="jd"), radius=4*u.pixel, magnitude_limit=18
    )
...
def test_bad_catalog():
    ...
    with pytest.raises(ValueError, match="Can not parse catalog name 'simbad'"):
        query_catalog(coord=c, epoch=epoch, catalog="simbad", radius=80, magnitude_limit=18)

def test_gaia():
    catalog_gaia = query_catalog(
        coord=c, epoch=Time(1569.4424277786259 + 2457000, scale="tdb", format="jd"), catalog="gaiadr3", radius=80, magnitude_limit=18
    )

    assert len(catalog_gaia) == 2

    catalog_gaia = query_gaia(
        coord=c, epoch=Time(1569.4424277786259 + 2457000, scale="tdb", format="jd"), radius=80, magnitude_limit=18
    )
...
def test_empty():
    catalog = query_catalog(SkyCoord.from_name('Kepler-10'),
              Time.now(),
              'epic',
